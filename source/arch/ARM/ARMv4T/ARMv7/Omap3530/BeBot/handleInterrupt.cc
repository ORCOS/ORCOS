	...
	switch (irqSrc)
	    {
			...
			case UART1_IRQ:
			{
				// the BeBot uses OMAP3530 UART 1 device as serial console
				LOG(ARCH,INFO,(ARCH,INFO,"UART 1 IRQ"));
			...
			case UART2_IRQ:
			{
				LOG(ARCH,INFO,(ARCH,INFO,"UART 2 IRQ."));
				#if HAS_Board_HCICfd
					register CommDeviceDriver* hciTransport = theOS->getBoard()->getHCI();
