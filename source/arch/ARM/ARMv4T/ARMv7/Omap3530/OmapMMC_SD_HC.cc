...
ErrorT OmapMMC_SD_HC::sendCommand(unint4 cmd, unint4 arg) {
    ...
    LOG(ARCH, WARN, "MMC/SD HC() CMD timed out");
...
ErrorT OmapMMC_SD_HC::readBlock(unint4 blockNum, unint1* buffer, unint4 length) {
    ...
    unint4 timeout = 3000;
    while (((INW(baseAddress + MMCHS_STAT) & (1 << 5)) == 0) && (timeout))
    {
        timeout--;
        kwait_us(10);
    };
...
ErrorT OmapMMC_SD_HC::writeBlock(unint4 blockNum, unint1* buffer, unint4 length) {
    ...
    unint4 timeout = 3000;
    while (((INW(baseAddress + MMCHS_STAT) & (1 << 4)) == 0) && (timeout))
    {
        timeout--;
        kwait_us(10);
    };
    ...
    timeout = 3000;
    while (((INW(baseAddress + MMCHS_STAT) & (STAT_TC)) == 0) && (timeout))
    {
        timeout--;
        kwait_us(10);
    };
