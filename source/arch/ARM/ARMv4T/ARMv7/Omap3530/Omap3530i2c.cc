...
ErrorT Omap3530i2c::i2c_read_byte(unint1 devaddr, unint1 regoffset, unint1 *values, unint1 length) {
    int i2c_error = cOk;
    unint2 status;
    ...
    // setup and clear FIFO
    unint2 clearcmd = (1 - 1) << 8 | I2C_BUF_RXFIFO_CLR | I2C_BUF_TXFIFO_CLR
            | (1 - 1);
    ...
    if (i2c_error == cOk)
    {
        ...
        unint2 buf = (1 - 1) << 8 | I2C_BUF_RXFIFO_CLR | I2C_BUF_TXFIFO_CLR
                | (1 - 1);
        OUTH(I2C_BUF, buf);
...
ErrorT Omap3530i2c::i2c_write_bytes(unint1 devaddr, const char* value, unint1 length) {
    ...
    // setup and clear FIFO
    unint2 buf = (unint2) ((length - 1) << 8 | I2C_BUF_RXFIFO_CLR
            | I2C_BUF_TXFIFO_CLR | (length - 1));
    OUTH(I2C_BUF, buf);
