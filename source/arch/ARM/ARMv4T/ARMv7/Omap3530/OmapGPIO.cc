...
OmapGPIO::OmapGPIO(T_OmapGPIO_Init *init) : CharacterDeviceDriver(true,init->Name)  {
    ...
    if (baseAddress == 0x48310000) gpionum = 1;
    if (baseAddress == 0x49050000) gpionum = 2;
    if (baseAddress == 0x49052000) gpionum = 3;
    if (baseAddress == 0x49054000) gpionum = 4;
    if (baseAddress == 0x49056000) gpionum = 5;
    if (baseAddress == 0x49058000) gpionum = 6;

    if (gpionum == -1) {
        LOG(ARCH,ERROR,(ARCH,ERROR,"OMAPGPIO invalid base address: %x",baseAddress) );
        return;
    }
