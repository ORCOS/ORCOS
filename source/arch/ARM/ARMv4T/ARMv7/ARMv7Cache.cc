...
ARMv7Cache::ARMv7Cache() {
	...
	LOG(ARCH,INFO,(ARCH,INFO,"ARMv7Cache: Type: 0x%x, CLID: 0x%x",type,clid));
}
...
void ARMv7Cache::invalidate_data(void* start, void* end) {
	...
	if (start_address & (line_len - 1)) {
		LOG(ARCH,WARN,(ARCH,WARN,"ARMv7Cache::invalidate_data() %s - start address is not aligned - 0x%08x",__func__, start));
	...
	if (end_address & (line_len - 1)) {
		LOG(ARCH,WARN,(ARCH,WARN,"ARMv7Cache::invalidate_data() %s - stop address is not aligned - 0x%08x", __func__, end));
		/* align to the beginning of this cache line */
		end_address &= ~(line_len - 1);
	}

	LOG(ARCH,INFO,(ARCH,INFO,"ARMv7Cache::invalidate_data() 0x%08x - 0x%08x", start_address, end_address));

	for (mva = start_address; mva < end_address; mva = mva + line_len) {
		/* DCIMVAC - Invalidate data cache by MVA to PoC */
		asm volatile ("mcr p15, 0, %0, c7, c6, 1" : : "r" (mva));
	}
	LOG(ARCH,INFO,(ARCH,INFO,"ARMv7Cache::invalidate_data() done"));
...
void ARMv7Cache::invalidate_instruction(void* start, void* end) {
	...
	unint4 mva;
		 ...
		 */
		if (start_address & (line_len - 1)) {
			LOG(ARCH,WARN,(ARCH,WARN,"ARMv7Cache::invalidate_instruction() %s - start address is not aligned - 0x%08x\n",__func__, start));
			/* move to next cache line */
			start_address = (start_address + line_len - 1) & ~(line_len - 1);
		}
		 ...
		 */
		if (end_address & (line_len - 1)) {
			LOG(ARCH,WARN,(ARCH,WARN,"ARMv7Cache::invalidate_instruction() %s - stop address is not aligned - 0x%08x\n", __func__, end));
			/* align to the beginning of this cache line */
			end_address &= ~(line_len - 1);
		}
