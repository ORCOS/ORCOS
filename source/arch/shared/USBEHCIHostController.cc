...
USB_EHCI_Host_Controller::USB_EHCI_Host_Controller(unint4 ehci_dev_base) :
        GenericDeviceDriver(true, "EHCI_HC") {
...
int USB_EHCI_Host_Controller::USBBulkMsg(USBDevice *dev, unint1 endpoint, unint1 direction, unint2 data_len, char *data) {
    ...
    volatile qTD* qtd = dev->endpoints[endpoint].qtd; //&qtds[(qtdnum++) & 0x7];
...
ErrorT USB_EHCI_Host_Controller::enumerateDevice(USBDevice *dev) {
    ...
    for (int i = 0; i < iDescr->bNumEndpoints; i++) {
        ...
        LOG(ARCH, INFO, "USB_EHCI_Host_Controller: Endpoint Adress: %d, Dir: %d, Transfer Size: %d", dev->endpoints[i + 1].address, dev->endpoints[i + 1].direction, dev->endpoints[i
                    + 1].interrupt_receive_size);
...
USBHub::USBHub(USBDevice *p_dev, USB_EHCI_Host_Controller *p_controller) :
        USBDeviceDriver() {
    this->dev = p_dev;
...
ErrorT USBDevice::deactivate() {
    for (int i = 0; i <= this->numEndpoints; i++) {
        ...
        if (endpoints[i].queue_head != 0)
            delete endpoints[i].queue_head;
        if (endpoints[i].qtd != 0)
            delete endpoints[i].qtd;
...
/*****************************************************************************
 * Method: USBDevice::reactivateEp(int num)
 *
 * @description
...
/*****************************************************************************
 * Method: USBDevice::activateEndpoint(int num, QH* qh = 0, qTD* qtd = 0)
 *
 * @description
