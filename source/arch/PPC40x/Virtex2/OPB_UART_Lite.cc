...
#include <arch/PPC40x/powerpc.h>
#include <arch/PPC40x/Virtex2/OPB_UART_Lite.hh>
#include <assembler.h>
...
/*---------------------------------------------------------------------------*/
{
    ...
    volatile unsigned char *port = (unsigned char *) getAddr();
...
/*---------------------------------------------------------------------------*/
ErrorT OPB_UART_Lite::readBytes( char* bytes, int4 &length )
/*---------------------------------------------------------------------------*/
{
...
/*---------------------------------------------------------------------------*/
ErrorT OPB_UART_Lite::writeBytes( const char* bytes, int4 length )
/*---------------------------------------------------------------------------*/
{
...
ErrorT OPB_UART_Lite::send( char* byte, int len, char* dest_addr, int addr_len, AddressProtocol* fromProtocol ) {
...
ErrorT OPB_UART_Lite::broadcast( char* bytes, int len, AddressProtocol* fromProtocol ) {
    return send( bytes, len, 0, 0, fromProtocol );
