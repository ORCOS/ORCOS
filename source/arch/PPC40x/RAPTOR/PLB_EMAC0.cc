...
PLB_EMAC0::PLB_EMAC0(T_PLB_EMAC0_Init* init) :
    ...
    *((volatile unsigned int *) (base_addr + MAC_HIGH_ADDR_OFFSET)) = MacAddr;
    ...
    MacAddr = ((LocalAddress)[3] << 24) | ((LocalAddress)[2] << 16) | ((LocalAddress)[1] << 8) | (LocalAddress)[0];
...
ErrorT PLB_EMAC0::handleIRQ() {
    ...
    while (status != (rx_frame_number)) {
        ...
        ByteCount = *((volatile unsigned int *) (base_addr + 4 * (rx_frame_number * FRAME_BEGIN) + RX_ADDR_OFFSET));
        ...
        for (unint i = 0; i < WordCount; i++) {
            WordBuffer[i] = *((volatile unsigned int *) (base_addr + RX_ADDR_OFFSET + 4 + 4 * i + 4 * (rx_frame_number * FRAME_BEGIN)));
        }
        ...
        if (ExtraByteCount > 0) {
            ...
            LastWord = *((volatile unsigned int *) (base_addr + RX_ADDR_OFFSET + 4 + 4 * WordCount + 4 * (rx_frame_number * FRAME_BEGIN)));
        ...
        *((volatile unsigned int *) (base_addr + CONTROL_REG_RX_OFFSET)) = rx_frame_number;
        ...
        if (ptBuf != 0) {
            ...
            LOG(ARCH, ERROR, "no memory for pbuf!");
        }
