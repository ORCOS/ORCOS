...
Task* TaskManager::getTask(int taskId) {
    ...
    for (LinkedListItem* lldi = this->taskDatabase->getHead();
            lldi != 0; lldi = lldi->getSucc())
    {
...
ErrorT handleTaskHeader(taskTable* taskCB, void* &taskHead, unint4 &nextHeader) {
    ...
    if (nextHeader == TASK_CB_CRC32)
    {
        ...
        if (crcHeader->crcEnd < crcHeader->crcStart) {
            LOG(PROCESS, ERROR,"CRC End < CRC Start!");
        ...
        unint4 start_address = ((unint4) taskCB)
                + (crcHeader->crcStart - (unint4) taskCB->task_start);
...
ErrorT TaskManager::checkValidTask(taskTable* taskCB) {
    ...
    if ((void*) taskCB->task_heap_start <= (void*) taskCB->task_start) {
...
void TaskManager::initialize() {
    ...
    for (unint1 i = 1; i <= num_tasks; i ++)
    {
        ...
        theOS->getHatLayer()->map((void*) LOG_TASK_SPACE_START, (void*) task_info, size, hatProtectionExecute
                                          | hatProtectionRead
                                          | hatProtectionWrite, 3, tid, !ICACHE_ENABLE);
        ...
        Kernel_MemoryManagerCfdCl* task_memManager =
                new (memaddr) Kernel_MemoryManagerCfdCl((void*) tt->task_heap_start, (void*) tt->task_end);
...
ErrorT TaskManager::removeTask(Task* task) {
    ...
    theOS->getDispatcher()->signal((void*) (task->getId() << 16), task->exitValue);
#endif
...
ErrorT TaskManager::loadTaskFromFile(File* file, TaskIdT& tid, char* arguments, unint2 arg_length) {
    ...
    /* be sure TLB entries are unmapped for ne new TID */
    ...
    theOS->getHatLayer()->map((void*) task_start,
                              (void*) task_start,
                              file->getFileSize(),
    ...
    register Kernel_MemoryManagerCfdCl* task_memManager =
            new (memaddr) Kernel_MemoryManagerCfdCl((void*) (tt->task_heap_start + DEFAULT_USER_STACK_SIZE + arg_length + 8 + RESERVED_BYTES_FOR_STACKFRAME),
                                                    (void*) tt->task_end);
    ...
    if (arguments != 0)
    {
        char* args = (char*) (task_start + (tt->task_heap_start - LOG_TASK_SPACE_START) + DEFAULT_USER_STACK_SIZE + RESERVED_BYTES_FOR_STACKFRAME + 4);
        ...
        thread->arguments = (void*) (tt->task_heap_start + DEFAULT_USER_STACK_SIZE + RESERVED_BYTES_FOR_STACKFRAME + 4);
    }
...
ErrorT  TaskManager::terminateThread( Kernel_ThreadCfdCl* pThread) {
    ...
    /* may not return if this is a periodic thread.
     * Otherwise we return and may cleanup other stuff */
    pThread->terminate();
