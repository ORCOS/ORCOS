...
SNServiceDiscovery::SNServiceDiscovery() :
    ...
    jobparam->time = theOS->getClock()->getTimeSinceStartup() + (1000ms);  // call the first time in 5 seconds
...
unint1 SNServiceDiscovery::nlookup(const service_name name, servicedescriptor* return_socks, unint1 n) {
...
bool SNServiceDiscovery::removeService(servicedescriptor* addr, bool local) {
    ...
    for (int i = 0; i < 10; i++)
    {
        if (services[i].address.sa_data == addr->address.sa_data)
        {
            ...
            if (strcmp(services[i].address.name_data, addr->address.name_data)
                    == 0)
            {
                // hit
...
void SNServiceDiscovery::handleBroadcast(servicedescriptor* sd, sockaddr* sender) {
    // copy the senders address into the service descriptor
    sd->type = SNSD_REMOTE;
    addService(sd, false);
}

void SNServiceDiscovery::handleRemoval(servicedescriptor* sd, sockaddr* sender) {
    // remove address
    sd->type = SNSD_REMOTE;
...
void SNServiceDiscovery::callbackFunc(void* param) {
    ...
    listenSocket =
            new Socket(cIPv4AddressProtocol, SOCK_DGRAM, cUDP, mysocketbuffer, 0x400);
    ...
    {
        len =
                listenSocket->recvfrom(pCurrentRunningThread, &msgptr, MSG_WAIT, &sender);
