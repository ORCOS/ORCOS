...
File* RamdiskDirectory::createFile(char* name, unint4 flags) {
    ...
    char* fileName = (char*) theOS->getMemoryManager()->alloc(strlen(name)+1);
...
ErrorT RamdiskFile::readBytes(char* bytes, unint4& length) {
    ...
    while (readlength > 0)
    {
        ...
        buffer = (char*) ((currentBlock * BLOCK_SIZE)
                + myRamDisk->getFirstBlockAddress());
