...
BufferDevice::BufferDevice(char* name, int bufferSize) : CharacterDevice(false,name) {
    buffersize = bufferSize;
    if (buffersize > MAX_BUFFER_SIZE)
        buffersize = MAX_BUFFER_SIZE;

    buffer      = (char*) theOS->getMemoryManager()->alloc(buffersize);
    usedBytes   = 0;
    readPointer = 0;
}
...
ErrorT BufferDevice::writeBytes(const char *bytes, unint4 length) {
    ...
    if (writeBytes == 0) return (cDeviceMemoryExhausted);
